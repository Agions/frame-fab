...
export const ExportPanel: React.FC<ExportPanelProps> = ({
  ...
  const hasBlockingIssues = qualityGate.issues.some((issue) => issue.level === 'error');
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        <Alert variant={qualityGate.passed ? 'default' : 'destructive'}>
          ...
          <AlertTitle>
            {qualityGate.passed ? '质量闸门通过，可进入导出流程' : '质量闸门未完全通过'}
          </AlertTitle>
          <AlertDescription>
                ...
                qualityGate.issues.map((issue) => (
                  <li key={issue.code} className="flex items-start gap-2">
                    ...
                    <span>
                      ...
                      {issue.field && (
                        <span className="text-muted-foreground"> 字段: {issue.field}</span>
                      )}
                    </span>
                  </li>
                ))
        ...
        <div className="flex justify-center">
          <Button onClick={onNavigateToEdit} disabled={hasBlockingIssues} className="gap-2">
