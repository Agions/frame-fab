...
PanelFlow 是一款基于大语言模型的**全自主 Agent 型**漫剧制作系统。用户只需提供原始文本（小说/剧本/需求描述），AI 将自主完成从剧本解析到成片输出的全部环节，真正实现**零参与、自动化**的漫剧生产体验。
