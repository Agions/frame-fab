...
const handleVideoUpload = async (files: File[]) => {
  for (const file of files) {
    if (file.size > 200 * 1024 * 1024) {   // ← 只是 maxSize 不同
...
const handleImageUpload = async (files: File[]) => {
  for (const file of files) {
    if (file.size > 10 * 1024 * 1024) {    // ← 只是 maxSize 不同
...
// ================================================================
// AFTER (zero duplication — uses @panel-flow/common)
// ================================================================
...
} from '@panel-flow/common';

import {
  useStepNavigation,
  useLocalStorage,
  useDebounce,
} from '@panel-flow/common/hooks';

import {
  validateCharacterName,
  validateScriptTitle,
} from '@panel-flow/common/domain/script/validators';
